...
- 2026-09-19 08:06:06 +07 — Replaced Remix's saved clone with AI recipe variation: a themed English/Vietnamese request sheet sends the full source recipe to the new Flask `/api/remix` endpoint, then opens an unsaved UUID draft for review. Preserves portions and source lineage; saves only on Done and leaves the original untouched. Added validation, retry/quota/connection errors, and dismissal handling; updated `APP_WALKTHROUGH.md`. Verified 7 backend tests, 8 client service tests, 3 localized modal tests, and 2 shopping regression tests with mocked AI (live Gemini generation not verified).
